...
export class ProviderQuotationsController {
  constructor(
    private readonly providerQuotationsService: ProviderQuotationsService,
  ) {}
  ...
  async getRequestDetail(
    ...
    try {
      console.log(
        '[ProviderQuotationsController] req.user:',
        JSON.stringify(req.user, null, 2),
      );
      const userId = req.user?.userId;
      const userRole = this.getUserRoleFromRequest(req);
      console.log('[ProviderQuotationsController] Extracted values:', {
        userId,
        userRole,
      });

      return this.providerQuotationsService.getRequestDetail(
        requestId,
        userId,
        userRole,
      );
  ...
  @ApiOperation({
    ...
    description:
      'Obtiene los pedidos aprobados que están disponibles para cotización y coinciden con las especialidades del proveedor',
  })
  @ApiQuery({
    name: 'specialty',
    required: false,
  ...
  async getAvailableRequests(
    ...
    try {
      ...
      const result = this.providerQuotationsService.getAvailableRequests(
        userId,
        userRole,
        { specialty, type, priority, page, limit },
      );

      // Log de las cotizaciones disponibles
      console.log('[ProviderQuotationsController] Cotizaciones disponibles:', {
        userId,
        userRole,
  ...
  @ApiOperation({
    ...
    description:
      'Permite a un proveedor cotizar total o parcialmente un pedido',
  })
  @ApiResponse({
    status: 201,
  ...
  @ApiOperation({
    ...
    description:
      'Lista las cotizaciones realizadas por el proveedor autenticado',
  })
  @ApiQuery({
    name: 'status',
    required: false,
  ...
  async getMyQuotations(
    ...
    try {
      ...
      return this.providerQuotationsService.getMyQuotations(userId, userRole, {
        status,
        dateFrom,
        dateTo,
        page,
  ...
  async getQuotationById(
    ...
    try {
      ...
      return this.providerQuotationsService.getQuotationById(
        id,
        userId,
        userRole,
      );
  ...
  async deleteQuotation(
    ...
    try {
      ...
      return this.providerQuotationsService.deleteQuotation(
        id,
        userId,
        userRole,
      );
  ...
  @ApiOperation({
    ...
    description:
      'Lista las cotizaciones del proveedor que han sido procesadas por auditores',
  })
  @ApiQuery({
    name: 'audit_result',
    required: false,
    description: 'Filtrar por resultado de auditoría',
  })
  @ApiQuery({
    name: 'auditor_id',
    required: false,
    description: 'Filtrar por auditor (solo admin)',
  })
  @ApiQuery({ name: 'page', required: false, type: Number })
  @ApiQuery({ name: 'limit', required: false, type: Number })
  @ApiResponse({
  ...
  async getAuditedQuotations(@Req() req: any, @Query() query: any) {
    try {
      const userId = req.user?.userId;
      const userRole = this.getUserRoleFromRequest(req);
  ...
  @ApiOperation({
    ...
    description:
      'Permite a un auditor revisar y aprobar/rechazar una cotización',
  })
  @ApiParam({ name: 'id', description: 'ID de la cotización' })
  @ApiResponse({
    status: 200,
  ...
  async auditQuotation(
    ...
    try {
      ...
      if (
        !['AUDITOR', 'ADMIN', 'Auditor', 'Administrador'].includes(userRole)
      ) {
        throw new HttpException(
      ...
      return this.providerQuotationsService.auditQuotation(
        id,
        auditDto,
        userId,
      );
  ...
  @ApiOperation({
    ...
    description:
      'Obtiene estadísticas generales para el dashboard del proveedor',
  })
  @ApiResponse({
    status: 200,
  ...
  async getProviderDashboardStats(@Req() req: any) {
    try {
      ...
      return this.providerQuotationsService.getProviderDashboardStats(
        userId,
        userRole,
      );
    } catch (error) {
