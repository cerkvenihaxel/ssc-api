...
export class ObraSocialService {
  ...
  async findById(healthcareProviderId: string): Promise<ObraSocial> {
    const obraSocial =
      await this.obraSocialRepository.findById(healthcareProviderId);
    if (!obraSocial) {
      throw new NotFoundException(
        `Obra Social con ID ${healthcareProviderId} no encontrada`,
      );
    }
    return obraSocial;
  }
  ...
  async create(dto: CreateObraSocialDto): Promise<ObraSocial> {
    ...
    const existingObraSocial = await this.obraSocialRepository.findByName(
      dto.name,
    );
    if (existingObraSocial) {
      throw new ConflictException(
        `Ya existe una obra social con el nombre ${dto.name}`,
      );
    }

    const obraSocial = ObraSocial.create(dto.name, dto.status, {
  ...
  async update(
    ...
    const updatedObraSocial = new ObraSocial(
      ...
      dto.contactPhone !== undefined
        ? dto.contactPhone
        : existingObraSocial.contactPhone,
      dto.contactEmail !== undefined
        ? dto.contactEmail
        : existingObraSocial.contactEmail,
